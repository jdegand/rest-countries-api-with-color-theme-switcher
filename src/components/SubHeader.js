...
function SubHeader(){
  ...
  const handleInputChange = (e) => {
    ...
    const handleSelectChange = (e) => {
      ...
      regionNames.forEach(item => {
        if(item.innerText.toLowerCase().includes(e.target.value.toLowerCase()) || e.target.value === "Filter by region"){
  ...
  return (
    <>
      <form id="form">
        <div>
          <i class="fas fa-search"></i><input type="search" name="search" placeholder="Search for a country..." onChange={handleInputChange} />
        </div>
        <div>
